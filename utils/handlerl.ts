...
export const wrapRequestHandler = (funcRegitter: RequestHandler) => {
  return async (req: Request, res: Response, next: NextFunction) => { //này có 3 tham số req: Request, res: Response, next: NextFunction gọi là request handler
    try {// và nếu request handler có lổi thì chỉ đẩy lổi qua cho erorr handler sử lý chứ nó không thể xuất lổi ra ngoài
